...
export class Dates {
  ...
  /**
   * ============================================================================
   * 1962 TRIDENTINE CALENDAR SPECIFIC DATE METHODS
   * ============================================================================
   */
  ...
  /**
   * Get the date of Septuagesima Sunday (9 weeks before Easter, 63 days before).
   * This is the start of the pre-Lenten season in the 1962 calendar.
   * @param year Gregorian year
   */
  septuagesimaSunday = (year = this.#year): Date => {
    if (this.#septuagesimaSunday[year]) return this.#septuagesimaSunday[year];
    return (this.#septuagesimaSunday[year] = subtractsDays(this.easterSunday(year), 63));
  };

  #septuagesimaSunday: Record<string, Date> = {};

  /**
   * Get the date of Sexagesima Sunday (8 weeks before Easter, 56 days before).
   * @param year Gregorian year
   */
  ...
  #sexagesimaSunday: Record<string, Date> = {};

  /**
   * Get the date of Quinquagesima Sunday (7 weeks before Easter, 49 days before).
   * This is the Sunday immediately before Ash Wednesday.
   * @param year Gregorian year
   */
  ...
  #quinquagesimaSunday: Record<string, Date> = {};
  ...
  /**
   * Get the number of Sundays after Epiphany in a given year (1-6).
   * The number varies based on the date of Easter.
   * @param year Gregorian year
   */
  numberOfSundaysAfterEpiphany = (year = this.#year): number => {
    if (this.#numberOfSundaysAfterEpiphany[year]) return this.#numberOfSundaysAfterEpiphany[year];
    const epiphanyOctaveDay = addDays(this.epiphany(year), 7); // Jan 13 or nearest Sunday
  ...
  /**
   * Get the date of Passion Sunday (2 weeks before Easter, 14 days before).
   * This marks the beginning of Passiontide, the last two weeks of Lent.
   * @param year Gregorian year
   */
  passionSunday = (year = this.#year): Date => {
    if (this.#passionSunday[year]) return this.#passionSunday[year];
    return (this.#passionSunday[year] = subtractsDays(this.easterSunday(year), 14));
  };

  #passionSunday: Record<string, Date> = {};
  ...
  /**
   * Get the dates of the Rogation Days (Monday, Tuesday, Wednesday before Ascension).
   * @param year Gregorian year
   */
  rogationDays = (year = this.#year): Date[] => {
    if (this.#rogationDays[year]) return this.#rogationDays[year];
    const ascension = this.ascension(year);
    return (this.#rogationDays[year] = [
      subtractsDays(ascension, 3), // Monday
      subtractsDays(ascension, 2), // Tuesday
      subtractsDays(ascension, 1), // Wednesday
    ]);
  };

  #rogationDays: Record<string, Date[]> = {};
  ...
  /**
   * Get the dates of the Ember Days in Advent (Wednesday, Friday, Saturday of 3rd week).
   * @param year Gregorian year
   */
  emberDaysAdvent = (year = this.#year): Date[] => {
    if (this.#emberDaysAdvent[year]) return this.#emberDaysAdvent[year];
    const thirdSundayAdvent = this.sundayOfAdvent(3, year);
    if (!thirdSundayAdvent) return (this.#emberDaysAdvent[year] = []);
    return (this.#emberDaysAdvent[year] = [
      addDays(thirdSundayAdvent, 3), // Wednesday
      addDays(thirdSundayAdvent, 5), // Friday
      addDays(thirdSundayAdvent, 6), // Saturday
    ]);
  };

  #emberDaysAdvent: Record<string, Date[]> = {};

  /**
   * Get the dates of the Ember Days in Lent (Wednesday, Friday, Saturday after Ash Wednesday).
   * @param year Gregorian year
   */
  emberDaysLent = (year = this.#year): Date[] => {
    if (this.#emberDaysLent[year]) return this.#emberDaysLent[year];
    const ashWed = this.ashWednesday(year);
    return (this.#emberDaysLent[year] = [
      ashWed, // Wednesday (Ash Wednesday itself)
      addDays(ashWed, 2), // Friday
      addDays(ashWed, 3), // Saturday
    ]);
  };

  #emberDaysLent: Record<string, Date[]> = {};

  /**
   * Get the dates of the Ember Days after Pentecost (Wed, Fri, Sat after Pentecost).
   * @param year Gregorian year
   */
  emberDaysPentecost = (year = this.#year): Date[] => {
    if (this.#emberDaysPentecost[year]) return this.#emberDaysPentecost[year];
    const pentecost = this.pentecostSunday(year);
    return (this.#emberDaysPentecost[year] = [
      addDays(pentecost, 3), // Wednesday
      addDays(pentecost, 5), // Friday
      addDays(pentecost, 6), // Saturday
    ]);
  };

  #emberDaysPentecost: Record<string, Date[]> = {};

  /**
   * Get the dates of the September Ember Days (Wed, Fri, Sat after Sept 14).
   * These fall after the Feast of the Exaltation of the Holy Cross (Sept 14).
   * @param year Gregorian year
   */
  emberDaysSeptember = (year = this.#year): Date[] => {
    if (this.#emberDaysSeptember[year]) return this.#emberDaysSeptember[year];
    const exaltation = this.exaltationOfTheHolyCross(year); // Sept 14
    const nextWednesday = addDays(exaltation, (3 - exaltation.getUTCDay() + 7) % 7);
    return (this.#emberDaysSeptember[year] = [
      nextWednesday, // Wednesday
      addDays(nextWednesday, 2), // Friday
      addDays(nextWednesday, 3), // Saturday
    ]);
  };

  #emberDaysSeptember: Record<string, Date[]> = {};

  /**
   * PENTECOST SEASON (replaces Ordinary Time in 1962)
   */

  /**
   * Get the number of Sundays after Pentecost in a given year (varies 23-28).
   * @param year Gregorian year
   */
  numberOfSundaysAfterPentecost = (year = this.#year): number => {
    if (this.#numberOfSundaysAfterPentecost[year]) return this.#numberOfSundaysAfterPentecost[year];
    const pentecost = this.pentecostSunday(year);
    const firstAdvent = this.firstSundayOfAdvent(year);
    const daysBetween = dateDifference(pentecost, firstAdvent);
    const sundayCount = Math.floor(daysBetween / 7);
    // Sundays after Pentecost range from 23-28 depending on Easter date
    return (this.#numberOfSundaysAfterPentecost[year] = Math.min(28, Math.max(23, sundayCount - 1)));
  };

  #numberOfSundaysAfterPentecost: Record<string, number> = {};

  /**
   * Get the date of a specific Sunday after Pentecost (1-28).
   * @param week Sunday number (1-28)
   * @param year Gregorian year
   */
  sundayAfterPentecost = (week: number, year = this.#year): Date | null => {
    const id = `${year}_${week}`;
    if (this.#sundayAfterPentecost[id] !== undefined) return this.#sundayAfterPentecost[id];
    if (week < 1 || week > 28) return (this.#sundayAfterPentecost[id] = null);
    const maxSundays = this.numberOfSundaysAfterPentecost(year);
    if (week > maxSundays) return (this.#sundayAfterPentecost[id] = null);
    const pentecost = this.pentecostSunday(year);
    return (this.#sundayAfterPentecost[id] = addDays(pentecost, 7 * week));
  };
  ...
  /**
   * ============================================================================
   * END OF 1962 SPECIFIC DATE METHODS
   * ============================================================================
   */
  ...
  startOfSeasons = (year = this.#year): Record<Season, Date> => {
    ...
    return (this.#startOfSeasons[year] = {
      ...
      [Season.AfterEpiphany]: addDays(this.epiphany(year), 8), // Day after Epiphany Octave
      [Season.SeptuagesimaTime]: this.septuagesimaSunday(year),
      [Season.Lent]: this.ashWednesday(year),
      [Season.Passiontide]: this.passionSunday(year),
      [Season.PaschalTriduum]: this.holyThursday(year),
      [Season.EasterTime]: this.easterSunday(year),
      [Season.PentecostSeason]: addDays(this.pentecostSunday(year), 1), // Monday after Pentecost
    });
  };
  ...
  endOfSeasons = (year = this.#year): Record<Season, Date> => {
    ...
    return (this.#endOfSeasons[year] = {
      ...
      [Season.ChristmasTime]: addDays(this.epiphany(year), 7), // Epiphany Octave Day
      [Season.AfterEpiphany]: subtractsDays(this.septuagesimaSunday(year), 1), // Day before Septuagesima
      [Season.SeptuagesimaTime]: subtractsDays(this.ashWednesday(year), 1), // Day before Ash Wednesday
      [Season.Lent]: subtractsDays(this.passionSunday(year), 1), // Day before Passiontide
      [Season.Passiontide]: subtractsDays(this.holyThursday(year), 1), // Day before Triduum
      [Season.PaschalTriduum]: this.easterSunday(year),
      [Season.EasterTime]: this.pentecostSunday(year),
      [Season.PentecostSeason]: subtractsDays(this.firstSundayOfAdvent(year), 1), // Day before Advent
    });
  };
